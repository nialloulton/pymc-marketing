...
def _get_distribution(self, dist: Dict) -> Callable:
    ...
    """
    distribution_name = dist.get("dist")
    if distribution_name == 'HSGP':
        # If the distribution is 'HSGP', call the custom build method
        return self.build_hsgp(dist)
    else:
        try:
            prior_distribution = getattr(pm, distribution_name)
        except AttributeError as e:
            raise ValueError(f"Distribution {distribution_name} does not exist in PyMC") from e
        return prior_distribution

    def build_hsgp(self, dist: Dict) -> Callable:
        """
        Build a custom prebuilt 'HSGP' distribution as a PyMC tensor variable.

        Parameters
        ----------
        dist : Dict
            A dictionary with parameters and configurations for building the HSGP distribution.

        Returns
        -------
        Callable
            A custom built HSGP distribution represented as a PyMC tensor variable.
        """
        # Custom logic for building the HSGP distribution goes here
        pass  # Replace with your custom HSGP logic
